...
class ReportPanel(QWidget):
    ...
    def _generate_report(self) -> None:
        ...
        report_text = []
        report_text.append("# Chest Tracker Correction Report")
        report_text.append("")
        report_text.append(f"Total entries: {len(self._entries)}")

        # Count entries by type
        type_counts = {}
        for entry in self._entries:
            chest_type = entry.chest_type
            if chest_type not in type_counts:
                type_counts[chest_type] = 0
            type_counts[chest_type] += 1

        report_text.append("")
        report_text.append("## Entries by Type")
        for chest_type, count in type_counts.items():
            report_text.append(f"- {chest_type}: {count}")

        # Count corrections
        correction_count = 0
        for entry in self._entries:
            # Check if the entry has corrections attribute
            if hasattr(entry, "corrections") and entry.corrections:
                correction_count += len(entry.corrections)

        report_text.append("")
        report_text.append("## Corrections")
        report_text.append(f"Total corrections: {correction_count}")

        # Display the report
        self._text_report.setText("\n".join(report_text))

        # Enable export button
        self._export_button.setEnabled(True)

        # Emit signal
        self.report_generated.emit(self._report_type_combo.currentData())
    ...
    def _generate_summary_report(self) -> None:
        ...
        for list_type, validation_list in self._validation_lists.items():
            self._add_table_row(
                f"{list_type.capitalize()} list entries", str(validation_list.count())
            )
    ...
    def _generate_player_stats_report(self) -> None:
        ...
        self._table_report.setRowCount(0)
        self._table_report.setColumnCount(4)
        self._table_report.setHorizontalHeaderLabels(
            ["Player", "Total Chests", "Corrected", "Validation Errors"]
        )
        ...
        for player, stats in sorted_players:
            ...
            sorted_chest_types = sorted(
                stats["chest_types"].items(), key=lambda x: x[1], reverse=True
            )
    ...
    def _generate_chest_types_report(self) -> None:
        ...
        self._table_report.setColumnCount(4)
        self._table_report.setHorizontalHeaderLabels(
            ["Chest Type", "Total", "Corrected", "Validation Errors"]
        )
        ...
        sorted_chest_types = sorted(chest_stats.items(), key=lambda x: x[1]["total"], reverse=True)
        ...
        for chest_type, stats in sorted_chest_types:
            ...
            sorted_players = sorted(stats["players"].items(), key=lambda x: x[1], reverse=True)
    ...
    def _generate_sources_report(self) -> None:
        ...
        self._table_report.setColumnCount(4)
        self._table_report.setHorizontalHeaderLabels(
            ["Source", "Total", "Corrected", "Validation Errors"]
        )
