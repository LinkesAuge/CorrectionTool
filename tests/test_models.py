...
class TestChestEntry:
    ...
    def test_initialization(self) -> None:
        ...
        # Check original values are empty since no corrections have been made
        assert not entry.original_values
    ...
    def test_apply_correction(self) -> None:
        ...
        # Check original values are preserved in the original_values dict
        assert entry.original_values.get("chest_type") == "Rare Chest of VVealth"
        assert entry.original_values.get("player") == "GUARDIENOfTHUNDER"
        assert "source" not in entry.original_values
    ...
    def test_to_text(self) -> None:
        ...
        entry = ChestEntry(
            chest_type="Cobra Chest", player="From: Engelchen", source="Level 15 Crypt"
        )
...
class TestCorrectionRule:
    ...
    def test_apply_to_text(self) -> None:
        ...
        rule = CorrectionRule(from_text="VV", to_text="W", rule_type="contains")
...
class TestValidationList:
    ...
    def test_initialization(self) -> None:
        ...
        player_list = ValidationList(list_type="player", entries=["Engelchen", "Sir Met", "Moony"])
    ...
    def test_is_valid(self) -> None:
        ...
        is_valid, _, _ = player_list.is_valid("Engelchen")
        assert is_valid is True

        is_valid, _, _ = player_list.is_valid("Unknown")
